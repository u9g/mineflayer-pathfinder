...
// Open state of a door or fence gate, from its block properties (boolean on 1.13+, 'true'/'false' on legacy metadata)
...
class Movements {
  constructor (bot) {
    ...
    registry.blocksArray.forEach(block => {
      ...
      if (name.endsWith('_door') && !name.includes('iron')) {
  ...
  getMoveForward (node, dir, neighbors) {
    ...
    if (this.canOpenDoors && blockC.openable) {
      // Gate or door: click it open if it is closed, then walk through
